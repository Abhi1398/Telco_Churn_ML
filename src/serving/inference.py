...
try:
    # For production: feature_columns.txt is in the same directory as model
    feature_file = os.path.join(MODEL_DIR, "feature_columns.txt")
    if not os.path.exists(feature_file):
        # For local development: feature_columns.txt is in artifacts/ directory (one level up from model/)
...
BINARY_MAP = {
    ...
    "SeniorCitizen": {0: 0, 1: 1, "0": 0, "1": 1},  # Senior citizen status (handles int and string)
